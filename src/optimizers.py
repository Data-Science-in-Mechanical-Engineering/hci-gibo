...
from src.acquisition_function import GradientInformation, MIGradientInformation
from src.model import ExactGPSEModel, DerivativeExactGPSEModel, MIDerivativeExactGPSEModel
...
class TrustBayesianGradientAscent(AbstractOptimizer):
    """Optimizer for HCI-BGD.
    ...
    def __init__(
        ...
        """Inits optimizer Bayesian gradient ascent."""
        super(TrustBayesianGradientAscent, self).__init__(params_init, objective)
        ...
        self.optimizer_torch = OptimizerTorch([self.params], **optimizer_torch_config)
        self.lr_schedular = lr_schedular
        self.lr = optimizer_torch_config['lr']
        self.lr_local = self.lr
        self.alpha = alpha
        self.iteration = 0
    ...
    def step(self) -> None:
        ...
        acq_value_old = None
        if self.lr_schedular:
            self.lr_local = [v for k, v in self.lr_schedular.items() if k <= self.iteration][-1]
        for i in range(self.max_samples_per_iteration):
            # Optimize acquistion function and get new observation.
            new_x, acq_value = self.optimize_acqf(self.acquisition_fcn, self.bounds)
            new_y = self.objective(new_x)
            ...
            self.model.posterior(self.params)
            self.acquisition_fcn.update_K_xX_dx()
            mean, var = self.model.posterior_derivative(self.params)
            product_distribution = torch.distributions.Normal(torch.norm(mean), torch.mm(mean / torch.norm(mean),
                                                                                         torch.mm(var[0],
                                                                                                  mean.transpose(0,
                                                                                                                 1) / torch.norm(
                                                                                                      mean))))
            L = self.model.covar_module.outputscale.detach().mean() / self.model.covar_module.base_kernel.lengthscale.detach().mean()
            if self.lr_schedular:
                p_trust = 1 - product_distribution.cdf(L * self.lr_local * torch.norm(mean) / 2)
            else:
                p_trust = 1 - product_distribution.cdf(L * self.lr * torch.norm(mean) / 2)

            if p_trust > self.alpha:
                if self.verbose:
                    print("Gradient step with trust.")
                break
...
class MIBayesianGradientAscent(AbstractOptimizer):
    """Optimizer for S-HCI-BGD.
    ...
    def __init__(
            ...
            hyperparameter_config: Optional[Dict[str, bool]],
            optimize_acqf: Callable[[MIGradientInformation, torch.Tensor, Dict[int, float]], torch.Tensor],
            optimize_acqf_config: Dict[str, Union[torch.Tensor, int, float]],
            bounds: Optional[torch.Tensor],
            delta: Optional[Union[int, float]],
            epsilon_diff_acq_value: Optional[Union[int, float]],
        ...
        """Inits optimizer Bayesian gradient ascent."""
        super(MIBayesianGradientAscent, self).__init__(params_init, objective)
        ...
        # Parameter initialization.
        self.params_history_list = [self.params.clone()]
        self.sampled_point_list = []
        self.params.grad = torch.zeros_like(self.params)
        self.D = self.params.shape[-1] - 1
        ...
        if (
            ...
            normalize = unnormalize = None
        self.model = Model(self.D + 1, normalize, unnormalize, **model_config)
        # Initialization of training data.
        if generate_initial_data is not None:
            train_x_init, train_y_init = generate_initial_data(self.objective)
            self.model.append_train_data(train_x_init, train_y_init)
        ...
        self.optimize_hyperparamters = hyperparameter_config["optimize_hyperparameters"]
        # Acquistion function and its optimization properties.
        self.acquisition_fcn = MIGradientInformation(self.model)
        self.optimize_acqf = lambda acqf, bounds, fixed_features: optimize_acqf(
            acqf, bounds, fixed_features, **optimize_acqf_config
        )
        self.bounds = bounds
        self.delta = delta
        self.update_bounds = self.bounds is None
    ...
    def step(self) -> None:
        ...
        if self.verbose:
            print(f"Reward of parameters theta_(t-1): {f_params.item():.2f}.")
        self.model.append_train_data(self.params, f_params)
        self.sampled_point_list.append(self.params)
        if (
                type(self.objective._func) is EnvironmentObjective
                and self.objective._func._manipulate_state is not None
                and self.objective._func._manipulate_state.apply_update() is not None
        ...
        # Stay local around current parameters.
        if self.update_bounds:
            self.bounds = torch.tensor([[-self.delta], [self.delta]]) + self.params[:, :-1]
        # Only optimize model hyperparameters if N >= N_max.
        if self.optimize_hyperparamters and (
                self.model.N >= self.model.N_max
        ):  # Adjust hyperparameters
        ...
        self.params[:, -1] = 0
        sample_budget = self.max_samples_per_iteration
        true_sampled_point = self.params.clone().detach()
        acq_value_old = 0
        IS = 1.0
        if self.lr_schedular:
            self.lr_local = [v for k, v in self.lr_schedular.items() if k <= self.iteration][-1]
        while True:
            # Optimize acquistion function and get new observation.
            if IS >= 0 and sample_budget > 0:
                new_x, acq_value = self.optimize_acqf(self.acquisition_fcn, self.bounds, fixed_features={self.D: IS})
                diff = acq_value - acq_value_old
                acq_value_old = acq_value
                if diff >= (self.epsilon_diff_acq_value / (self.D)) or IS == 0:
                    new_y = self.objective(new_x)
                    if IS > 0:
                        self.objective._calls -= 1
                    self.model.append_train_data(new_x, new_y)
                    self.sampled_point_list.append(new_x)
                    if IS == 0:
                        sample_budget = sample_budget - 1
                    mean, var = self.model.posterior_derivative(true_sampled_point)

                    product_distribution = torch.distributions.Normal(torch.norm(mean),
                                                                      torch.mm(mean / torch.norm(mean), torch.mm(var[0],
                                                                                                                 mean.transpose(
                                                                                                                     0,
                                                                                                                     1) / torch.norm(
                                                                                                                     mean))))
        ...
        with torch.no_grad():
            ...
            if self.lr_schedular:
                ...
                self.params.grad[:, :-1] = lr * params_grad  # Define as gradient ascent.
            else:
                self.params.grad[:, :-1] = params_grad  # Define as gradient ascent.
            self.optimizer_torch.step()
            self.iteration += 1
        ...
        if self.verbose:
            ...
            print(
                f"lengthscale_IS: {self.model.covar_module_IS.base_kernel.lengthscale.detach().numpy()}, outputscale_IS: {self.model.covar_module_IS.outputscale.detach().numpy()}"
            )