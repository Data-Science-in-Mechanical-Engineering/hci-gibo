...
class MIExactGPModel(gpytorch.models.ExactGP, botorch.models.gpytorch.GPyTorchModel):
    """An exact Gaussian process (GP) model with a squared exponential (SE) kernel.
    ...
    def __init__(
        ...
        train_y: torch.Tensor,
        lengthscale_constraint=None,
        lengthscale_constraint_IS=None,
        lengthscale_hyperprior=None, #gpytorch.priors.NormalPrior(.24, 0.2),
        lengthscale_hyperprior_IS=None, #gpytorch.priors.NormalPrior(.24, 0.2),
        outputscale_constraint=None,
        outputscale_constraint_IS=None,
        outputscale_hyperprior=None, #gpytorch.priors.NormalPrior(0.8, 0.1),
        outputscale_hyperprior_IS=None, #gpytorch.priors.NormalPrior(0.8, 0.1),
        noise_constraint=None, #gpytorch.constraints.Interval(0.0008, 0.0012),
        noise_hyperprior=None, #gpytorch.priors.UniformPrior(0.0008, 0.0012),
        ard_num_dims=None,
        prior_mean=0,
    ):
        """Inits GP model with data and a Gaussian likelihood."""
        ...
        if train_y is not None:
            train_y = train_y.squeeze(-1)
        super(MIExactGPModel, self).__init__(train_x, train_y, likelihood)
        self.mean_module = gpytorch.means.ConstantMean()
        if prior_mean != 0:
            self.mean_module.initialize(constant=prior_mean)
            self.mean_module.constant.requires_grad = False
        ...
        self.covar_module_IS = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.RBFKernel(
                ard_num_dims=ard_num_dims,
                lengthscale_prior=lengthscale_hyperprior_IS,
                lengthscale_constraint=lengthscale_constraint_IS,
            ),
            outputscale_prior=outputscale_hyperprior_IS,
            outputscale_constraint=outputscale_constraint_IS,
        )
        # Initialize lengthscale and outputscale to mean of priors.
        if lengthscale_hyperprior is not None:
            self.covar_module.base_kernel.lengthscale = lengthscale_hyperprior.mean
        if outputscale_hyperprior is not None:
            self.covar_module.outputscale = outputscale_hyperprior.mean
        if lengthscale_hyperprior_IS is not None:
            self.covar_module_IS.base_kernel.lengthscale = lengthscale_hyperprior_IS.mean
        if outputscale_hyperprior is not None:
            self.covar_module_IS.outputscale = outputscale_hyperprior_IS.mean

    def get_delta(self, train_x):
        delta_list = train_x[:, -1].clone().detach()
        dim = delta_list.size(dim=0)
        delta = torch.mul(delta_list.view(-1, dim), delta_list.view(dim, -1))
        return delta
    ...
    def forward(self, x):
        ...
        x = x[:, :-1]
        mean_x = self.mean_module(x)
        covar_x = self.covar_module(x) + delta.mul(self.covar_module_IS(x).evaluate())
        return gpytorch.distributions.MultivariateNormal(mean_x, covar_x)
...
class MIDerivativeExactGPSEModel(MIExactGPModel):
    """Derivative of the MIExactGPSEModel w.r.t. the test points x.
    ...
    Attributes:
        ...
        N_max: Maximum number of training samples (train_x, N) for model inference.
        lengthscale_constraint: Constraint for lengthscale of SE-kernel, gpytorch.constraints.
        lengthscale_constraint_IS: Constraint for the additional IS lengthscale of SE-kernel, gpytorch.constraints.
        lengthscale_hyperprior: Hyperprior for lengthscale of SE-kernel, gpytorch.priors.
        lengthscale_hyperprior: Hyperprior for the additional IS lengthscale of SE-kernel, gpytorch.priors.
        outputscale_constraint: Constraint for outputscale of SE-kernel, gpytorch.constraints.
        outputscale_constraint_IS: Constraint for the additional IS outputscale of SE-kernel, gpytorch.constraints.
        outputscale_hyperprior: Hyperprior for outputscale of SE-kernel, gpytorch.priors.
        outputscale_hyperprior_IS: Hyperprior for the additional IS outputscale of SE-kernel, gpytorch.priors.
        noise_constraint: Constraint for noise, gpytorch.constraints.
        noise_hyperprior: Hyperprior for noise, gpytorch.priors.
        ard_num_dims: Set this if you want a separate lengthscale for each input dimension.
            Should be D if train_x is a N x D matrix.
    ...
    def __init__(
        ...
        super(MIDerivativeExactGPSEModel, self).__init__(
            train_x_init,
            train_y_init,
            lengthscale_constraint,
            lengthscale_constraint_IS,
            lengthscale_hyperprior,  # gpytorch.priors.NormalPrior(.24, 0.2),
            lengthscale_hyperprior_IS, #gpytorch.priors.NormalPrior(.24, 0.2),
            outputscale_constraint,
            outputscale_constraint_IS,
            outputscale_hyperprior,  # gpytorch.priors.NormalPrior(0.8, 0.1),
            outputscale_hyperprior_IS,  # gpytorch.priors.NormalPrior(0.8, 0.1),
            noise_constraint,
            noise_hyperprior,
            ard_num_dims,
            prior_mean,
    ...
    def append_train_data(self, train_x, train_y):
        ...
        self.train_ys = torch.cat([train_y, self.train_ys])
            ...
            #self.train_xs = self.train_xs[: self.N_max]
            #self.train_ys = self.train_ys[: self.N_max]
            ## To-do: update hyperparameters
        t = self.train_xs[:, -1]
        if t[t==1].size(0) > self.N_max and t[t==0].size(0) > self.N_max:
            clip = t[t==0].size(0) + t[t==1].size(0)
            self.train_xs = self.train_xs[: clip]
            self.train_ys = self.train_ys[: clip]
        
        self.set_train_data(
            inputs=self.normalize(self.train_xs),
            targets=self.train_ys.view(-1),
            strict=False,
        )
        self.N = self.train_xs.shape[0]

    def purge_train_data(self, num):
        """Adaptively purge training data.
        ...
        Args:
            ...
            train_y: (1 x 1) New training target.
        """
        self.train_xs = self.train_xs[num:, :]
        self.train_ys = self.train_ys[num:, :]
    ...
    def get_L_lower_old(self):
        X = self.train_inputs[0]
        delta = self.get_delta(X)
        X = X[:, :self.D]
        return torch.cholesky(
            self.covar_module(X).evaluate() + delta.mul(self.covar_module_IS(X).evaluate())
        )
    ...
    def get_KXX_inv_old(self):
        ...
        X = X[:, :self.D]
        return torch.inverse(
            self.covar_module(X).evaluate() + delta.mul(self.covar_module_IS(X).evaluate()) + torch.eye(X.shape[0]) * sigma_n
        )
    ...
    def _get_KxX_dx(self, x):
        ...
        X = self.train_inputs[0]
        n = x.shape[0]
        delta = self.get_delta(torch.cat([x, X], dim=0))
        delta = delta[:n, n:]
        delta = delta.unsqueeze(1)
        K_xX = self.covar_module(x[:, :self.D], X[:, :self.D]).evaluate()
        K_xX_IS = self.covar_module_IS(x[:, :self.D], X[:, :self.D]).evaluate()
        x = x[:, :self.D]
        X = X[:, :self.D]
        lengthscale = self.covar_module.base_kernel.lengthscale.detach()
        lengthscale_IS = self.covar_module_IS.base_kernel.lengthscale.detach()
        return (
            -torch.eye(self.D, device=x.device)
            / lengthscale ** 2
            @ (
            ...
            - delta.mul(torch.eye(self.D, device=x.device)
            / lengthscale_IS ** 2
            @ (
                (x.view(n, 1, self.D) - X.view(1, self.N, self.D))
                * K_xX_IS.view(n, self.N, 1)
            ).transpose(1, 2))
        )
    ...
    def _get_Kxx_dx2(self):
        ...
        return (
            torch.eye(self.D, device=lengthscale.device) / lengthscale ** 2
        ) * sigma_f  # We always need the policy for the ground truth.
    ...
    def posterior_derivative(self, x):
        ...
        #    self.posterior(x)  # Call this to update prediction strategy of GPyTorch.
        K_xX_dx = self._get_KxX_dx(x)
        mean_d = K_xX_dx @ self.get_KXX_inv_old() @ self.train_targets
        variance_d = (
            self._get_Kxx_dx2() - K_xX_dx @ self.get_KXX_inv_old() @ K_xX_dx.transpose(1, 2)
        )
        variance_d = variance_d.clamp_min(1e-9)
