...
import botorch
import gpytorch
from src.model import MIExactGPModel
...
def sample_from_gp_prior_IS(
    ...
    gp_hypers_IS:  Dict[str, Tensor] = {
        "covar_module.base_kernel.lengthscale": torch.tensor(0.8),
        "covar_module.outputscale": torch.tensor(0.2),
    },
    gp_hypers_IS1: Dict[str, Tensor] = {
        "covar_module.base_kernel.lengthscale": torch.tensor(0.8),
        "covar_module.outputscale": torch.tensor(0.2),
    },
    gp_hypers_IS2: Dict[str, Tensor] = {
        "covar_module.base_kernel.lengthscale": torch.tensor(0.8),
        "covar_module.outputscale": torch.tensor(0.2),
    },
    gp_hypers_IS3: Dict[str, Tensor] = {
        "covar_module.base_kernel.lengthscale": torch.tensor(0.8),
        "covar_module.outputscale": torch.tensor(0.2),
    },
    ...
    model = ExactGPModel(None, None, likelihood, ard_num_dims=dim)
    model.initialize(**gp_hypers)
    model_IS = ExactGPModel(None, None, likelihood, ard_num_dims=dim)
    model_IS.initialize(**gp_hypers_IS)
    model_IS1 = ExactGPModel(None, None, likelihood, ard_num_dims=dim)
    model_IS1.initialize(**gp_hypers_IS1)
    model_IS2 = ExactGPModel(None, None, likelihood, ard_num_dims=dim)
    model_IS2.initialize(**gp_hypers_IS2)
    model_IS3 = ExactGPModel(None, None, likelihood, ard_num_dims=dim)
    model_IS3.initialize(**gp_hypers_IS3)
    train_x = irregular_grid(dim)(num_samples)
    train_y = sample_from_gp_prior_helper(model)(train_x)
    train_y_IS = sample_from_gp_prior_helper(model_IS)(train_x)
    train_y_IS1 = sample_from_gp_prior_helper(model_IS1)(train_x)
    train_y_IS2 = sample_from_gp_prior_helper(model_IS2)(train_x)
    train_y_IS3 = sample_from_gp_prior_helper(model_IS3)(train_x)
    return train_x, train_y, train_y_IS, train_y_IS1, train_y_IS2, train_y_IS3
...
def generate_objective_from_gp_post_IS(
    train_x: Tensor,
    train_y: Tensor,
    train_y_IS: Tensor,
    noise_variance: float = 1e-6,
    ...
    """Generate objective function (dual information source) with given train_x, train_y and hyperparameters.
    ...
    #train_y_IS = train_y_IS.view(-1, 1)
    #train_x = torch.cat([train_x, -1 * torch.ones(train_x.size(dim=0), 1)], dim=1)
    likelihood = gpytorch.likelihoods.GaussianLikelihood()
    model = ExactGPModel(None, None, likelihood, ard_num_dims=dim)
    model.initialize(**gp_hypers)
    model.set_train_data(inputs=train_x, targets=train_y, strict=False)
    ...
    model_IS.initialize(**gp_hypers_IS)
    #train_x = torch.cat([train_x, 1 * torch.ones(train_x.size(dim=0), 1)], dim=1)
    model_IS.set_train_data(inputs=train_x, targets=train_y_IS, strict=False)
    ...
    def objective(x, observation_noise=True, requires_grad=False):
        ...
        with gpytorch.settings.fast_pred_var():
            posterior_IS = model_IS(x[:, :-1])
            m_IS = posterior_IS.mean.flatten()

        IS_list = x[:, -1].clone().detach()
        if requires_grad:
            return (m + IS_list.mul(m_IS)).view(-1, 1)
        else:
            return (m + IS_list.mul(m_IS)).view(-1, 1).detach()
...
'''
'''
def generate_training_samples_IS(
    num_objectives: int, dim: int, num_samples: int, gp_hypers: Dict[str, Tensor], gp_hypers_IS: Dict[str, Tensor], gp_hypers_IS1: Dict[str, Tensor], gp_hypers_IS2: Dict[str, Tensor], gp_hypers_IS3: Dict[str, Tensor]
) -> Tuple[List[Tensor], List[Tensor], List[Tensor], List[Tensor], List[Tensor], List[Tensor]]:
    """Generate training samples for `num_objectives` objectives (dual information sources).
    ...
    Args:
        ...
        num_samples: Number of grid samples.
        gp_hypers: GP model hyperparameters.
        gp_hypers_IS: GP model hyperparameters for the additional information sources.
    Returns:
        List of trainings features and targets.
    """
    ...
    train_y_IS3 = []
    for _ in range(num_objectives):
        x, y_true, y_IS, y_IS1, y_IS2, y_IS3 = sample_from_gp_prior_IS(dim, num_samples, gp_hypers, gp_hypers_IS, gp_hypers_IS1, gp_hypers_IS2, gp_hypers_IS3)
        train_x.append(x)
        train_y.append(y_true)
        train_y_IS.append(y_IS)
        train_y_IS1.append(y_IS1)
        train_y_IS2.append(y_IS2)
        train_y_IS3.append(y_IS3)
    return train_x, train_y, train_y_IS, train_y_IS1, train_y_IS2, train_y_IS3
